...
const parseTrainType = (_trainType: string | string[]): string => {
  const trainType = Array.isArray(_trainType)
    ? _trainType
    : _trainType.split(",");
...
const generateUrl = (query: NextApiRequest["query"]): string => {
  ...
  url.searchParams.set(
    "productsFilter",
    parseTrainType(stringifyQuery(query, "trainType")),
  );
...
const api: NextApiHandler = async (req, res) => {
  ...
  const journeyItems = parseJourneys(
    html,
    stringifyQuery(req.query, "type") as "dep" | "arr",
  );
