...
export async function GET(request: Request) {
  ...
  const { ConfirmationKey, __RequestVerificationToken } = await getKeyAndToken(number, date);
  ...
  const res = await fetch("https://mersultrenurilor.infofer.ro/en-GB/Trains/TrainsResult", {
    headers: {
      accept: "*/*",
...
async function getKeyAndToken(number: string, date: string) {
  const url = new URL(`https://mersultrenurilor.infofer.ro/en-GB/Train/${number}`);
...
function parseStations(html: string): Station[] {
  ...
  const stations = $(".mt-3 .list-group li.list-group-item")
    ...
    .map((element): Station => {
      ...
      const date = $element.find(".col-md-5 a").attr("href")?.split("Date=")[1] ?? "";
      const dateObj = dayjs(date, "DD/MM/YYYY");
      const dateYmd = dateObj.isValid() ? dateObj.format("YYYY-MM-DD") : getRomaniaDate();
      const platform = $element.find(".col-6.col-md-8 .col-md-2:last-child").text().trim();
      ...
      const departureDelay = departureElement.find(".text-0-8rem").text().trim();
...
function parseName(html: string): string {
  const $ = cheerio.load(html);
  return ($("h2").text().split(" in ")[0] ?? "").trim().replace(/(\t|\s)+/, " ");
}
