  ...
  const journey = $journeyRows
    .map((index, elm) => {
      ...
      const trainType = parseTrainType(
        $elm.find(".train:nth-child(2) img").attr("src") || "",
      );
      const train = $elm
        .find(".train:nth-child(3)")
        .text()
        .trim()
        .replace(/\s+/, " ");
      const detailHref = parseDetailHref(
        $elm.find(".train:nth-child(3) a").attr("href") || "",
      );
      const originOrDestination = $elm
        .find(".route > .bold:first-child")
        .text()
        .trim();
      ...
      const accurateStation =
        ($elm.find(".platform").html()?.split("<br>")[1]?.trim() || null) ??
        null;
      const actualTime =
        ($elm.find(".ris > .delay, .ris > .delayOnTime").text().trim() ||
          null) ??
        null;
      ...
      const journey: Journey = {
        ...
        detailHref,
        arrivalDelayed: type === "arr" ? delayed : false,
        departureDelayed: type === "dep" ? delayed : false,
        trainType,
        train,
...
const parseMessage = (
  ...
  const title =
    $row.find(".route .himMessagesHigh .himMessage").text().trim() || null;
  ...
  const onClick = $row
    .find(".route .himMessagesHigh .arrowlink")
    .attr("onclick");
  const onClickMatches = onClick?.match(
    /himMessagesAjaxRequest\('.+','(.+)',this\)/,
  );
...
const extractInformation = (info: string[]): JourneyInformation => {
  return {
    ...
    replacedTo:
      info
        .find((i) => i.match(/^Es verkehrt Ersatzfahrt\s/))
        ?.replace(/^Es verkehrt Ersatzfahrt\s/, "") ?? null,
    ...
    changedOriginTo:
      info
        .find((i) => i.match(/^Fährt heute erst ab\s/))
        ?.replace(/^Fährt heute erst ab\s/, "") ?? null,
    changedDestination: info.some((i) => i.match(/^Fährt heute nur bis\s/)),
    changedDestinationTo:
      info
        .find((i) => i.match(/^Fährt heute nur bis\s/))
        ?.replace(/^Fährt heute nur bis\s/, "") ?? null,
    specialTrain: info.includes("Sonderfahrt"),
    replacementTrain: info.some((i) => i.match(/^Ersatzfahrt für\s/)),
    replacementTrainFrom:
      info
        .find((i) => i.match(/^Ersatzfahrt für\s/))
        ?.replace(/^Ersatzfahrt für\s/, "") ?? null,
    others: info.filter((i) => {
      ...
      if (
        [
          /^Es verkehrt Ersatzfahrt\s/,
          /^Ersatzfahrt für\s/,
          /^Fährt heute erst ab\s/,
          /^Fährt heute nur bis\s/,
        ].some((regexp) => i.match(regexp))
      ) {
        return false;
      }
      return true;
