...
export const Main: React.FC<MainProps> = ({ className }) => {
  ...
  const markers = useMemo<(MarkerProps & { key: string })[]>(() => {
    return stations.map((station) => {
      const categoryNumber = Number.parseInt(
        station.dbRisStationCateogry.replace(/^CATEGORY_/, "") ?? "7",
        10,
      );
      const label = { 1: "1", 2: "2", 3: "3", 4: "4", 5: "", 6: "", 7: "" }[
        categoryNumber
      ];
      const opacity = { 1: 1, 2: 0.9, 3: 0.8, 4: 0.6, 5: 0.4, 6: 0.4, 7: 0.4 }[
        categoryNumber
      ];
      return {
  ...
  const onClickMap = (e: google.maps.MapMouseEvent) => {
    ...
    const placeId =
      "placeId" in e && e.placeId && typeof e.placeId === "string"
        ? e.placeId
        : undefined;
        ...
        <StationMap
          ...
          onClickMarker={(stationId) => {
            const station = stations.find(
              (station) => station.dbRisStationId === stationId,
            );
